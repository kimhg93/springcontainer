...
public class VideoSystemXmlConfigTest {
	...
	@Autowired
	@Qualifier("captaionAmerica")
	DigitalVideoDisc dvd6;
	
	@Autowired
	@Qualifier("avengersDirectorsEdition")
	...
	public void testDvd7Null() {
		...
		assertEquals("BlankDisc [title=Avengers Director's Edition, studio=MARVEL, "
					+"actors=[Robert Downey Jr., Chris Hemsworth, Mark Ruffalo]]",systemOutRule.getLog().replace("\r\n", "").replace("\n", ""));
	}
	@Test
	public void testDvd8Null() {
		assertNotNull(dvd8);
		System.out.println(dvd8);
		assertEquals("BlankDisc [title=Avengers Expansion Pack, studio=MARVEL, "
					+"actors=[Robert Downey Jr., Chris Hemsworth, Mark Ruffalo]]",systemOutRule.getLog().replace("\r\n", "").replace("\n", ""));
	}
	@Test
	public void testDvd9Null() {
		...
		assertEquals("BlankDisc [title=Avengers Expansion Pack2, studio=MARVEL, "
					+"actors=[Robert Downey Jr., Chris Hemsworth, Mark Ruffalo]]",systemOutRule.getLog().replace("\r\n", "").replace("\n", ""));
	}
	@Test
